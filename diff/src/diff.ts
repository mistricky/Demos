...
function walk(oldVdomNode:any, newVdomNode:any, index:number, patches:any){
  ...
  } else if (isString(oldVdomNode) && isString(newVdomNode) && oldVdomNode != newVdomNode){
    // 字符串
    currentPatches.push({
...
function childrenDiff(oldVdomChildren:any, newVdomChildren:any,index:number,patches:any){
...
function attrsDiff(oldAttrs:Attrs, newAttrs:Attrs):Attrs{
  ...
  for(let attr of Object.keys(union)){
    if(!oldAttrs.hasOwnProperty(attr) || !newAttrs.hasOwnProperty(attr) || oldAttrs[attr] !== newAttrs[attr]){
